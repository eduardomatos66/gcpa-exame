    ...
    <div class="card-q" v-else>
      <span v-if="!startQuiz">
        <b-card
            ...
            img-alt="Image"
            img-top
            title="Google Cloud Platform Architect - Practice Exam"
...
export default {
  ...
  methods: {
    async getQuestions() {
      try {
        const {data} = await QuestionService.getQuestions(this.numberOfQuestions, labels=['Architect', this.selectedLabel])
        this.questions = data;
